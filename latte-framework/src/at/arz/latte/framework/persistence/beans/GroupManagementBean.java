...
public class GroupManagementBean {
	...
	public List<PermissionData> getAllPermissionsData() {
		return em.createNamedQuery(Permission.QUERY_GETALL_BASE,
				PermissionData.class).getResultList();
	}
	...
	private void setGroupPermissions(Group group, Set<PermissionData> permissionData) {
	 ...
	 */
	private void checkDuplicateGroup(Long groupId, String name)
			throws LatteValidationException {
		List<Group> duplicates = em
				.createNamedQuery(Group.QUERY_GET_BY_NAME, Group.class)
				.setParameter("name", name).getResultList();

		if (duplicates.size() == 1
				&& !duplicates.get(0).getId().equals(groupId)) {
			throw new LatteValidationException(400, "name",
